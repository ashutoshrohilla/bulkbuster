...
/*
 Bulk Buster DoS tool on <strike>steroids</strike> goroutines. Just ported from Python with some improvements.
 Original Python utility by Ashutosh Rohilla
*/
...
func main() {
	...
	flag.StringVar(&data, "data", "", "Data to POST. If present bulkbuster will use POST requests instead of GET")
	flag.Var(&headers, "header", "Add headers to the request. Could be used multiple times")
	flag.Parse()

	t := os.Getenv("BBMAXPROCS")
	...
	if version {
		fmt.Println("Bulkbuster", __version__)
		os.Exit(0)
	}
